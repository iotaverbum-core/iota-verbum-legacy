...
def _read_html(path: Path) -> str:
    try:
        ...
        return f"<pre>Could not read {path.name}: {e}</pre>"
...
@app.get('/cne', response_class=HTMLResponse)
def cne_encoder():
    """Serve the CNE encoder UI (static HTML)."""
    return _read_html(ROOT / 'cne_encoder.html')
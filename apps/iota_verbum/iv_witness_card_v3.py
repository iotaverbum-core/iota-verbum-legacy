...
def load_template_v3(passage_ref: str):
    templates_root = resolve_repo_path("data/templates/witness_cards/v3")
    ...
    for path in [v2_root / f"{passage_slug}.json", v2_root / f"{book}.json", v2_root / "nt.json", v2_root / "generic.json"]:
        if path.exists():
            ...
            template["_template_path"] = _to_repo_relative(path)
            return template
    raise FileNotFoundError("No template found for v3 witness card generation.")
...
def build_evidence_map(thresholds, silences, utterances, prompts):
    evidence = {
        "thresholds": [],
        "silences": [],
        "utterances": [],
...
def generate_witness_card_v3(
    ...
    template = load_template_v3(passage_ref)
    ...
    card_base = f"v3|{passage_ref}|{normalized_text}|{moment}"
    ...
    scene["evidence_map"] = build_evidence_map(
        thresholds_detailed, silences_detailed, utterances_detailed, prompt_links
    )

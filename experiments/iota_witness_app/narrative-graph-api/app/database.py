...
class PopularPassage(Base):
    __tablename__ = "popular_passages"
    ...
    last_analyzed: Mapped[datetime] = mapped_column(DateTime, default=lambda: datetime.now(UTC).replace(tzinfo=None))
...
class Database:
    ...
    async def record_analysis(self, job_data: dict[str, Any]) -> None:
        async with self.session_factory() as session:
            session.add(AnalysisJob(**job_data))

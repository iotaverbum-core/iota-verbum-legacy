...
try:
    ...
    def stop_after_attempt(_count: int) -> None:
        return None

    def wait_exponential(**_kwargs: Any) -> None:
        return None

from app.models import LogosReference, NarrativeGraph
...
class LogosBridge:
    ...
    def _transform(self, raw: dict[str, Any], ref: LogosReference) -> dict[str, Any]:
        ...
        return {
            ...
            "text": raw.get("text", ""),
            "verses": verses,
            "metadata": {"fetched_at": datetime.now(UTC).isoformat(), "provider": "logos"},
        }
    ...
    async def format_for_logos(self, graph: NarrativeGraph) -> dict[str, Any]:
        ...
        nodes_html = "".join(f"<li><strong>{n.type}:</strong> {n.text}</li>" for n in graph.nodes[:8])
        html_panel = (
            "<div class='narrative-panel'><h3>Narrative Flow</h3>"
            f"<p>{graph.primary_reference}</p><div>{motifs_html}</div><ul>{nodes_html}</ul></div>"
        )
        json_data = {
            "graph_id": graph.id,
            "reference": graph.primary_reference,
            "statistics": {"nodes": len(graph.nodes), "edges": len(graph.edges), "motifs": len(graph.motifs)},
            "motifs": [m.model_dump() for m in graph.motifs],
        }
        export_formats = {
            "sermon": json.dumps({"points": [n.text for n in graph.nodes if n.type == "identity"]}),
            "study": json.dumps({"questions": [f"How does this matter? {m.name}" for m in graph.motifs]}),

...
class LogosReference(BaseModel):
    resource_id: str = Field(
        ..., description="Logos resource ID (e.g., 'LSS:ESV', 'WBC.Mark')"
    )
    passage: str = Field(..., description="Bible reference (e.g., 'Mark 5:21-43')")
...
class NarrativeGraph(BaseModel):
    ...
    metadata: dict[str, Any] = Field(
        default_factory=lambda: {
            ...
            "version": NARRATIVE_API_VERSION,
            "source": "narrative-graph-api",
        }
    )
...
class NarrativeResponse(BaseModel):
    ...
    error: str | None = None
    api_version: str = NARRATIVE_API_VERSION
...
class NarrativeAnalyzeRequest(BaseModel):
    passage: str = Field(..., examples=["Mark 5:21-43"])
    text: str | None = None
